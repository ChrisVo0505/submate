    ...
    if ($admin['smtp_address'] && $admin['smtp_port'] && $admin['smtp_username'] && $admin['smtp_password'] && $admin['encryption']) {
        ...
        try {
            foreach ($rows as $user) {
                ...
                $mail->Body = '<img src="' . $server_url . 'public_html/app/images/icon/logo.png" alt="Logo" />
                    <br>
                    Registration on Submate was successful.
                    <br>
                    Please click the following link to verify your email: 
                    <a href="' . $server_url . 'public_html/app/verifyemail.php?email=' . $user['email'] . '&token=' . $user['token'] . '">Verify Email</a>';
